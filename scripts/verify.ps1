...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-parse-regex-detail.ps1

ruby -Ilib tests/test_domain_review.rb

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-parse-regex-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-parse-regex-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper_parse_regex_stack-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
